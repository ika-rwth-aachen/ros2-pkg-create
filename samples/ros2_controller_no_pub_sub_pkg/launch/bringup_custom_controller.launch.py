...
def generate_launch_description() -> LaunchDescription:
    ...
    # custom_controller spawner
    custom_controller_launch = GroupAction(
        actions=[
            # Include the controller manager nodes
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    [
                        str(Path(get_package_share_directory("ros2_controller_no_pub_sub_pkg")) / "launch" / "custom_controller.launch.py"),
                    ]
                ),
            ),
        ],
    ...
    controller_manager_launch = GroupAction(
        actions=[
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    [
                        str(Path(get_package_share_directory("ros2_controller_no_pub_sub_pkg")) / "launch" / "controller_manager.launch.py"),
                    ]
                ),
            ),
        ],
    ...
    robot_description_launch = GroupAction(
        actions=[
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    [
                        str(Path(get_package_share_directory("ros2_controller_no_pub_sub_pkg")) / "launch" / "robot_description.launch.py"),
                    ]
                ),
                launch_arguments={
